...
{
    public partial class AdicionarColheitaSangueDiagPrecoce : Form
    {
        ...
        public AdicionarColheitaSangueDiagPrecoce(Paciente pac)
        {
            InitializeComponent();
            paciente = pac;
        ...
        private void AdicionarColheitaSangueDiagPrecoce_Load(object sender, EventArgs e)
        {
            idAtitude();
            if (id == -1)
            {
                ...
                if (resposta == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        string queryInsertData = "INSERT INTO Atitude(nomeAtitude) VALUES('Colheita de sangue diagnóstico precoce');";
        ...
        private void idAtitude()
        {
            try
            {
                conn.Open();
                com.Connection = conn;
                SqlCommand cmd = new SqlCommand("select * from Atitude WHERE nomeAtitude = 'Colheita de sangue diagnóstico precoce'", conn);
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            ...
            string idadeDias = txtIdadeDias.Text;
            ...
            if (VerificarDadosInseridos())
            {
                try
                {
                    ...
                    string queryInsertData = "INSERT INTO ColheitadeSanguePrecoce(IdAtitude,IdPaciente,data,idadeDias,observacoes) VALUES(@id,@IdPaciente,@dataR,@idade,@obs);";
                    SqlCommand sqlCommand = new SqlCommand(queryInsertData, connection);
                    sqlCommand.Parameters.AddWithValue("@IdPaciente", paciente.IdPaciente);
                    sqlCommand.Parameters.AddWithValue("@dataR", dataRegisto.ToString("MM/dd/yyyy"));
                    sqlCommand.Parameters.AddWithValue("@idade", idadeDias);
                    ...
                    sqlCommand.ExecuteNonQuery();
                    MessageBox.Show("Colheita de sangue diagnóstico precoce registada com Sucesso!", "Sucesso!", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    connection.Close();
                    limparCampos();
                }
                catch (SqlException)
                {
                    ...
                    MessageBox.Show("Por erro interno é impossível registar a Colheita de sangue diagnóstico precoce!", "Atenção!", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
        ...
        private Boolean VerificarDadosInseridos()
        {
            ...
            string idade = txtIdadeDias.Text;
            ...
            {
                MessageBox.Show("Campo obrigatório, por favor preencha a idade!", "Atenção!", MessageBoxButtons.OK, MessageBoxIcon.Error);

                if (txtIdadeDias.Text == string.Empty)
                {
                    errorProvider.SetError(txtIdadeDias, "A idade é obrigatória!");
                }
                else
                {
                    errorProvider.SetError(txtIdadeDias, String.Empty);
                }
                return false;
            }
            ...
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("select * from ColheitadeSanguePrecoce WHERE IdPaciente = @IdPaciente AND IdAtitude = @id", conn);
                ...
                while (reader.Read())
                {
                    ...
                    {
                        MessageBox.Show("Não é possível registar a Colheita de Sangue de Diagnóstico Precose, porque já está um registo na data que selecionou! \n Selecione outra data!", "Informação", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        conn.Close();
                        return false;
                    }
        ...
        private void txtIdadeDias_KeyPress(object sender, KeyPressEventArgs e)
        {
        ...
        {
            VerColheitaSangueDiagPrecoce verColheitaSangueDiagPrecoce = new VerColheitaSangueDiagPrecoce(paciente);
            verColheitaSangueDiagPrecoce.Show();
        }
    }
}
